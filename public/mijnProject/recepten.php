
<?php
require('php/begin.php');
require('db.php');
?>
                <h1>Zoete recepten</h1>
                <hr>
                <div id= "a">
                    <div class="een">
                        <img src="pics/Koekies.png" alt="recept1" height="80%" style="margin: auto" >
                        <br><br>
                        <a href="recept/recept1.php">Zandkoekjes</a>
                    </div>
                    <div class="twee">
                        <img src="pics/applcake.png" alt="recept2" height="80%" style="margin: auto" >  
                        <br><br>
                        <a href="recept/recept2.php">Appelcake</a>                  
                    </div>
                </div>
                <div id="b">
                    <div class="drie">
                        <img src="pics/tompoes.jpg" alt="recept3" height="80%" style="margin: auto" >
                        <br><br>
                        <a href="recept/recept3.php">Aardbeientompoes</a>
                    </div>
                    <div class="vier">
                        <img src="pics/chocotaart.png" alt="recept4" height="80%" style="margin: auto" >
                        <br><br>
                        <a href="recept/recept4.php">Chocoladetaart</a>
                    </div>
                </div> 
                <h1>Hartige recepten</h1>
                <hr>   
                <div id="c">
                    <div class="vijf">
                        <img src="pics/broccoli.png" alt="recept5" height="80%" style="margin: auto" >
                        <br><br>
                        <a href="recept/recept5.php">Broccoliquiche</a>
                    </div>
                    <div class="zes">
                        <img src="pics/nacho.png" alt="recept6" height="80%" style="margin: auto" >
                        <br><br>
                        <a href="recept/recept6.php">Nachos</a>
                    </div>
                </div>
                <div id="d">
                    <div class="zeven">
                        <img src="pics/broodtaart.png" alt="recept7" height="80%" style="margin: auto" >
                        <br><br>
                        <a href="recept/recept7.php">Hartige broodtaartjes</a>
                    </div>
                    <div class="acht">
                        <img src="pics/hartigtaart.png" alt="recept8" height="80%" style="margin: auto" >
                        <br><br>
                        <a href="recept/recept8.php">Hartige taart met tonijn</a>
                    </div>
                    </div>
<?php
require('php/eind.php');
?>